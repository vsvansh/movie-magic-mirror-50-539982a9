...
const NavBar = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${isScrolled ? 'bg-netflix-black' : 'bg-gradient-to-b from-black/70 to-transparent'}`}>
      <div className="flex items-center justify-between px-4 md:px-8 lg:px-12 py-4">
        {/* Logo */}
        <div className="flex items-center gap-8">
          <Link to="/" className="h-5 md:h-7">
            <svg viewBox="0 0 111 30" className="h-full w-28 md:w-32 fill-netflix-red">
              <g>
          ...
          <nav className="hidden md:flex items-center gap-4">
            ...
            <Link to="/new" className="text-sm text-white hover:text-gray-300 transition">New & Popular</Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <select 
              className="bg-transparent border border-white/30 text-white py-1 px-2 text-sm rounded appearance-none pr-8 hover:border-white/70 transition-colors"
              value={language}
          ...
          <Button 
            size="sm"
            className="bg-netflix-red hover:bg-[#f40612] text-sm font-medium px-4 py-1"
          >
            Sign In
          </Button>
        </div>
